...
Implementation of the VertexFitDump, in the beginning only a old-fashioned primary vertex finder
is implemented
...
#include "VertexFitDump.h"

VertexFitDump::VertexFitDump()
{
...
int VertexFitDump::setRecEvent(SRecEvent* recEvent, int sign1, int sign2)
{
    ...
    for(int i = 0; i < nPos; ++i)
    {
        ...
        for(; j < nNeg; ++j)
        {
			...
			z_vertex_opt = -199.332 + 27.9488*m - 3.66186*m*m + 0.175479*m*m*m;
...
double VertexFitDump::findDimuonVertexFast(SRecTrack& track1, SRecTrack& track2)
{
...
void VertexFitDump::setStartingVertex(double z_start, double sigz_start)
{
...
int VertexFitDump::processOnePair()
{
...
void VertexFitDump::addTrack(int index, KalmanTrack& _track)
{
...
void VertexFitDump::addTrack(int index, SRecTrack& _track)
{
    TrkPar _trkpar;
...
void VertexFitDump::addTrack(int index, TrkPar& _trkpar)
{
    _trkpar_curr.push_back(_trkpar);
}

int VertexFitDump::findVertex()
{
    int nIter = 0;
...
void VertexFitDump::updateVertex()
{
...
double VertexFitDump::findSingleMuonVertex(SRecTrack& _track)
{
...
double VertexFitDump::findSingleMuonVertex(Node& _node_start)
{
    TrkPar _trkpar_start;
...
double VertexFitDump::findSingleMuonVertex(TrkPar& _trkpar_start)
{
...
void VertexFitDump::bookEvaluation(std::string evalFileName)
{
...
void VertexFitDump::fillEvaluation()
{
